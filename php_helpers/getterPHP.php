<?php
    ...
    function getStudentSubjectsAndTeachers($data)
    {
        $grade_level = $data["grade_level"];
        $section = $data["section"];
        $res = selectDatabase("SELECT handle.ID, subjects.subject_name, sections.grade_level, teacher.first_name, teacher.last_name FROM sections JOIN handle ON sections.ID=handle.section_ID JOIN subjects ON subjects.ID=handle.subject_ID JOIN teacher ON teacher.ID=handle.teacher_ID WHERE sections.grade_level=$grade_level AND sections.section_name='$section' AND handle.active=1");
        return $res;
    }

    function getStudentSubjectActivities($data)
    {
        $handle_ID = $data["handle_ID"];
        $LRN = $data["LRN"];
        $sqlWhere = "subject_activity.handle_ID=";
        if(count($handle_ID)==1){
            $sqlWhere .= $handle_ID[0];
        }else{
            for ($i=0; $i < count($handle_ID); $i++) { 
                ...
                if($i==count($handle_ID)-1){
                    ...
                    $sqlWhere .= $handle_ID[$i].' OR subject_activity.handle_ID=';
                }
            }
        }
        $res = selectDatabase("SELECT student_activity.ID, subject_activity.handle_ID, student_activity.activity_ID, subject_activity.activity_title, subject_activity.category, student_activity.status FROM subject_activity JOIN student_activity ON subject_activity.ID=student_activity.activity_id WHERE ($sqlWhere) AND student_activity.LRN='$LRN' AND subject_activity.active=1 ORDER BY subject_activity.ID DESC");
        return $res;
    }
    ...
    function getActivityCount($data)
    {
        ...
        $res = selectDatabase("SELECT COUNT(student_activity.ID) AS Total FROM student_activity JOIN subject_activity ON student_activity.activity_id=subject_activity.ID WHERE LRN='$LRN' AND subject_activity.active=1 AND subject_activity.section_name='$section_name' AND subject_activity.grade_level=$grade_level AND subject_activity.category='Assignment'");
        $statistics["Assignment"]["Total"] = $res[0]["Total"];
        $res = selectDatabase("SELECT COUNT(student_activity.ID) AS Total FROM student_activity JOIN subject_activity ON student_activity.activity_id=subject_activity.ID WHERE LRN='$LRN' AND subject_activity.active=1 AND subject_activity.section_name='$section_name' AND subject_activity.grade_level=$grade_level AND subject_activity.category='Assignment' AND student_activity.status=1");
        $statistics["Assignment"]["Finished"] = $res[0]["Total"];
        if($statistics["Assignment"]["Total"] == 0){
            $statistics["Assignment"]["Percent"] = 100;
            $divisor = $divisor - 1;
        ...
        $res = selectDatabase("SELECT COUNT(student_activity.ID) AS Total FROM student_activity JOIN subject_activity ON student_activity.activity_id=subject_activity.ID WHERE LRN='$LRN' AND subject_activity.active=1 AND subject_activity.section_name='$section_name' AND subject_activity.grade_level=$grade_level AND subject_activity.category='Quiz'");
        $statistics["Quiz"]["Total"] = $res[0]["Total"];
        $res = selectDatabase("SELECT COUNT(student_activity.ID) AS Total FROM student_activity JOIN subject_activity ON student_activity.activity_id=subject_activity.ID WHERE LRN='$LRN' AND subject_activity.active=1 AND subject_activity.section_name='$section_name' AND subject_activity.grade_level=$grade_level AND subject_activity.category='Quiz' AND student_activity.status=1");
        $statistics["Quiz"]["Finished"] = $res[0]["Total"];
        if($statistics["Quiz"]["Total"] == 0){
            $statistics["Quiz"]["Percent"] = 100;
            $divisor = $divisor - 1;
        ...
        $statistics["Performance Task"]["Total"] = $res[0]["Total"];
        $res = selectDatabase("SELECT COUNT(student_activity.ID) AS Total  FROM student_activity JOIN subject_activity ON student_activity.activity_id=subject_activity.ID WHERE LRN='$LRN' AND subject_activity.active=1 AND subject_activity.section_name='$section_name' AND subject_activity.grade_level=$grade_level AND subject_activity.category='Performance Task'AND student_activity.status=1");
        $statistics["Performance Task"]["Finished"] = $res[0]["Total"];
        if($statistics["Performance Task"]["Total"] == 0){
            $statistics["Performance Task"]["Percent"] = 100;
            $divisor = $divisor - 1;
